...
def noise_slide_window(total_w, total_impact, B_w_impact_loss=None, I_w_impact_loss=None, h=7, w=10, c=4, batch_size=2, residual_proportion=(1./5.)):
    ...
    for i in range(1, video_len+1):
        ...
        if i < k:
            ...
            w_impact_loss = torch.matmul(torch.ones_like(total_noise[:, :1, :1, :1]), total_w[:, :i, :, :])

            # calculate the variance of the new noise
            new_B_w_impact_loss = torch.sum((w_impact_loss * total_B_impact[..., :i, :, :]) ** 2, dim=(4, 5, 6)).squeeze()
            new_I_w_impact_loss = torch.sum((w_impact_loss * total_I_impact[..., :i, :, :]) ** 2, dim=(4, 5, 6)).squeeze()
            ...
            w_impact_loss = torch.matmul(torch.ones_like(total_noise[:, :1, :1, :1]), total_w[:, i-k:i, :, :])

            new_B_w_impact_loss = torch.sum((w_impact_loss * total_B_impact) ** 2, dim=(4, 5, 6)).squeeze()
            new_I_w_impact_loss = torch.sum((w_impact_loss * total_I_impact) ** 2, dim=(4, 5, 6)).squeeze()

        B_noise_loss_group.append(new_B_w_impact_loss.clone())
        I_noise_loss_group.append(new_I_w_impact_loss.clone())


    B_noise_group = torch.stack(B_noise_group, dim=0)
    I_noise_group = torch.stack(I_noise_group, dim=0)

    B_w_impact_loss = torch.stack(B_noise_loss_group, dim=-1)
    I_w_impact_loss = torch.stack(I_noise_loss_group, dim=-1)

    B_noise_group =  B_noise_group.permute(1, 0, 5, 4, 2, 3)
    I_noise_group =  I_noise_group.permute(1, 0, 5, 4, 2, 3)

    return B_noise_group, I_noise_group, B_w_impact_loss, I_w_impact_loss
