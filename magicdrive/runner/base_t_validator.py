...
from magicdrive.pipeline.pipeline_bev_controlnet_t import (
...
class BaseTValidator(BaseValidator):
    def construct_visual(self, images_list, val_input, with_box):
        ...
        if with_box:
            ...
            frames_wb = None
        return frames, frames_wb, frame_list, frame_list_wb
    ...
    def validate(
        ...
        w_bi, impact, residual_proportion
    ):
        logging.info(f"[{self.__class__.__name__}] Running validation... ")
        torch.cuda.empty_cache()
        pipeline = self.prepare_pipe(controlnet, unet, unet_I, weight_dtype, device)
        ...
        for validation_i in self.cfg.runner.validation_index:
            ...
            def mask_gen(coord, x_scale=1, y_scale=1):
                # Calculating mask
            ...
            def run_once(pipe_param):
                for _ in range(self.cfg.runner.validation_times):
                    with torch.autocast("cuda"):
                        latents_mask = mask_gen(val_input['meta_data']['coord'], 8, 8).unsqueeze(2).half()[:val_input['pixel_values'].shape[0], ...]
                    ...
                    gen_frames, gen_frames_wb, gen_frames_list, gen_frames_wb_list = self.construct_visual(
                ...
                return gen_frames_list, gen_frames_wb_list
            ...
            if self.cfg.runner.pipeline_param['init_noise'] != 'both':
                gen_frames_list, gen_frames_wb_list = run_once(pipeline_param)
            else:
            ...
            ori_img, ori_img_wb, ori_img_list, ori_img_wb_list = self.construct_visual(
