...
class BaseRunner:
    ...
    def _init_fixed_models(self, cfg):
        ...
        self.unet_I = UNet2DConditionModel.from_pretrained(cfg.model.pretrained_model_name_or_path, subfolder="unet")
    ...
    def _set_xformer_state(self):
        ...
        if self.cfg.runner.enable_xformers_memory_efficient_attention:
            if is_xformers_available():
                ...
                self.unet_I.enable_xformers_memory_efficient_attention()
                # self.mmodel.enable_xformers_memory_efficient_attention()
    ...
    def _set_gradient_checkpointing(self):
        if hasattr(self.cfg.runner.enable_unet_checkpointing, "__len__"):
            ...
            self.unet_I.enable_gradient_checkpointing(
                self.cfg.runner.enable_unet_checkpointing)
    ...
    def run(self):
        ...
        if self.cfg.resume_from_checkpoint:
            ...
            if path is None:
                ...
                elif "SDv1.5mv-rawbox-t_2023-12-04_17-51_2.0t_0.4.3E4-S77040" == path:
                    magicdrive_unet_model_path = f"{self.cfg.resume_from_checkpoint}/weight-E4-S77040/unet/diffusion_pytorch_model.bin"
                    magicdrive_control_net_path = f"{self.cfg.resume_from_checkpoint}/weight-E4-S77040/controlnet/diffusion_pytorch_model.bin"

                    self.unet_I.load_state_dict(torch.load(magicdrive_unet_model_path, map_location='cuda'))
                    self.unet.load_state_dict(torch.load(magicdrive_unet_model_path, map_location='cuda'))
                    self.controlnet.load_state_dict(torch.load(magicdrive_control_net_path, map_location='cuda'))
                    initial_global_step = 0
                    pass
                else:
                    self.accelerator.print(
                        f"first training !!!!! Resuming from checkpoint {self.cfg.resume_from_checkpoint}"
                    )
                    self.accelerator.wait_for_everyone()  # wait
        ...
        if self.cfg.validation_only:
            if self.cfg.resume_from_checkpoint is None:
                raise ValueError("Validation need resume_from_checkpoint, but it is None!!!")
            logging.info(f"initial_global_step : {initial_global_step}")
            logging.info(f"validation.init_w_bi : {self.init_w_bi}")
            self.w_bi = self.mmodel(self.init_w_bi)
            self._validation(initial_global_step, 0.7)
            return
        # val before train 
        ...
        for epoch in range(first_epoch, self.cfg.runner.num_train_epochs):
            for step, batch in enumerate(self.train_dataloader):
                # Slowly increase the residual ratio to reduce the difference with the pretrain model
                if global_step <= 1000:
                    residual_proportion = 1. - (int(global_step/100) *0.03)
                else:
                    residual_proportion = 7. / 10.
                logging.info(f"residual_proportion : {residual_proportion}")
                loss, tensorboard_logs = self._train_one_step(batch, residual_proportion)
                ...
                if self.accelerator.sync_gradients:
                    ...
                    if global_step % self.cfg.runner.checkpointing_steps == 0:
                        ...
                        self._save_model(f"{save_path}_model")

                    # validation
                    if self.accelerator.is_main_process:
                        if global_step % self.cfg.runner.validation_steps == 0:
                            self._validation(global_step, residual_proportion)

                logs = {"loss": loss.detach().item()}
                logs.update(tensorboard_logs)
    ...
    def _validation(self, step, residual_proportion):
        controlnet = self.accelerator.unwrap_model(self.controlnet)
        unet = self.accelerator.unwrap_model(self.unet)
        unet_I = self.accelerator.unwrap_model(self.unet_I)

        with torch.no_grad():
            image_logs = self.validator.validate(
                controlnet, unet, unet_I, self.accelerator.trackers, step,
                self.weight_dtype, self.accelerator.device, self.w_bi, self.impact, residual_proportion)
